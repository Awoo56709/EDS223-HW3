---
title: "Texas Blackouts and Socioeconomic Lenses"
...
# Warning and Error Verification

```{r}
# Warning if no blackout polygons detected
if (nrow(blackout_sf) == 0) {
  warning(sprintf("No blackout areas detected at the given threshold = %s. This may indicate the wrong threshold or misaligned tiles.", threshold))
}

# Error if homes layer is missing
if (!exists("homes") || nrow(homes) == 0) {
  stop("No residential building data found. Please check that the buildings layer was loaded correctly.")
}
```
...
ggplot(pre_df, aes(x = x, y = y, fill = VNP46A1.A2021038.h08v05.001.2021039064328.tif)) +
  ...
  labs(title = "Night Light Radiance Before Storm (Feb 7)", fill = "VIIRS Radiance (nW/cm^²/sr)") +
  theme_minimal() +
...
ggplot(post_df, aes(x = x, y = y, fill = VNP46A1.A2021047.h08v05.001.2021048091106.tif)) +
  ...
  coord_sf(xlim = c(-96.5, -94.5), ylim = c(29, 30.5)) +
  scale_fill_viridis_c(option = "E") +
  labs(title = "Night Light Radiance After Storm (Feb 16)", fill = "VIIRS Radiance (nW/cm^²/sr)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
# Estimating the Number of Homes Affected by the Storm
...
```{r}
affected_homes_message <- sprintf("Estimated homes that lost power: %s", scales::comma(n_homes_lost_power))

# Print the formatted estimate text
print(affected_homes_message)

```

## Estimating of the Number of Homes in Houston that Lost Power

We estimate that approximately 168,874 homes in Houston lost power during the February 2021 storms.
...
ggplot() +
  ...
  geom_sf(data = blackout_clean, aes(fill = "Blackout area"), color = NA, alpha = 0.4) +
  geom_sf(data = homes_no_power, aes(fill = "Homes without power"), color = NA, alpha = 0.6) +
  scale_fill_manual(values = c("Blackout area" = "red", "Homes without power" = "black")) +
  labs(title = "Areas and Homes in Houston that Lost Power",
       fill = "Legend") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
These maps and estimates suggest substantial drops in night lights during the February 2021 storms, with census tracts overlapping residential areas indicating power loss. By excluding highways and validating blackout polygons, we estimate that thousands of homes lost power. These renders are useful in top level visualization. The night lights visualizations paint a clearer picture of the magnitude Houston was affected as a community. It is estimated that about 168,864 homes were affected. The income distributions hint at potential disparities in impacts as the median income plot above indicates a higher proportion of those affected by blackouts had a lower median income. The blackout distribution illustrates that those with lower median income experienced a blackout and decreased as median income increases.
